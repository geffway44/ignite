...
trait Searchable
{
    ...
    {
        $comparator = 'mysql' != $this->getDatabaseDriver() ? implode(' + ', $selects) : $this->getRelevanceField();

        $relevance_count = number_format($relevance_count, 2, '.', '');

        if ('mysql' == $this->getDatabaseDriver()) {
            $bindings = [];
        } else {
    ...
    {
        $like_comparator = 'pgsql' == $this->getDatabaseDriver() ? 'ILIKE' : 'LIKE';
        $cases = [];
    ...
    {
        if ('pgsql' == $this->getDatabaseDriver()) {
